// src/services/auth/cognito/cognito.verify.ts
  ...
  return new Promise((resolve, reject) => {
    jwt.verify(token, getKey, { algorithms: ["RS256"], issuer }, (err, decoded) => {
      ...
      if (p?.token_use !== opts.expectedUse) {
        return reject(
          new Error(`Invalid token_use. Expected ${opts.expectedUse}, got ${p?.token_use}`)
        );
      }

      // bind to client app
      if (opts.expectedUse === "access") {
        if (p?.client_id !== opts.clientId) return reject(new Error("Invalid client_id"));
      } else {
        if (p?.aud !== opts.clientId) return reject(new Error("Invalid aud"));
      }
